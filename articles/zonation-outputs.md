...
This vignette demonstrates how to explore and interpret some of the main
outputs generated by Zonation using the *ZonationR* package.
Specifically, users will:

- Examine the spatial patterns of conservation priorities using the
  **priority rank map**.
- Inspect **performance curves** to evaluate how features are
  represented across the prioritization ranking.
- Analyze the **distribution of feature coverage** within top-priority
  areas.
...
priority. But please note that **the scale is relative, not absolute**.
For example, a cell ranked 0.8 is not twice as valuable as one ranked
0.4. The priority map should then be considered together with other
outputs, like performance curves, to fully understand the spatial
distribution of conservation value, potential trade-offs, and the
conservation effectiveness. We can use the `plot_rankmap()` function to
visualize the priority rank map across the landscape:
...
`classify` argument in `plot_rankmap()` lets us choose whether to
display the map as continuous or divided into classes. If we use
`classify = TRUE`, the map is divided into discrete classes, and we can
define custom breaks and labels to make it easier to see which areas
...
The visualization functions in *ZonationR* return `ggplot` objects,
which means that users can further customize the plots using the
`ggplot2` ecosystem. For example, we can apply a different color palette
to the map:
...
The performance curves describe how much of each feature’s distribution
would be covered if we protected grid cells following their priority
rank order. The
[`summary_curves()`](https://thiago-cav.github.io/ZonationR/reference/summary_curves.md)
function is helpful for understanding the overall performance of a
solution. It allows us to explore key metrics such as:
...
[`summary_curves()`](https://thiago-cav.github.io/ZonationR/reference/summary_curves.md),
one could also be interested in how each feature is represented across
the priority ranking, which can be explored using the
...
and
[`feature_curves()`](https://thiago-cav.github.io/ZonationR/reference/feature_curves.md)
return **ggplot** objects, allowing further customization of the plots.
For example, we can add a vertical line to highlight the top 10%
...
> The vertical line at 0.9 represents the top 10% of the landscape.
> Intersection with the curves shows the proportion of each feature
> representation within these high-priority areas.

As mentioned earlier, priority rank maps and performance curves should
be interpreted together. For example, we could use the
[`priority_map()`](https://thiago-cav.github.io/ZonationR/reference/priority_map.md)
function to plot a classified map showing only the top 10% priority
...
represented within those areas (i.e., the curves intersecting the red
dashed line), providing a complete view of spatial priorities and
species coverage.
...
function. The histogram shows, for each coverage level (or coverage bin)
on the x-axis, the number of species reaching that coverage level
(y-axis). For example, a bar reaching 30 on the y-axis at the coverage
range 0.1-0.2 means that 30 species have between 10% and 20% of their
range included in the selected priority fraction:
...
top-priority (**map**), how well each species is represented in those
areas (**feature curves**), and the number of species at different
coverage levels (**histogram**). This provides a full view of spatial
priorities and feature-level coverage, illustrating how *ZonationR*
allows users to explore and interpret conservation outcomes in a
flexible and intuitive way.  
